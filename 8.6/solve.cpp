                    ...
                    const color original_color)
{
	...
	while (Q.empty() == false)
	{
		...
		if (R.x+1 < I.width() && I(R.x+1,R.y) == original_color && explored(R.x+1,R.y) == false)
		{
			Q.push(pixel{R.x+1,R.y});
			explored(R.x+1,R.y) = true;
		}
		if (R.x > 0 && I(R.x-1,R.y) == original_color && explored(R.x-1,R.y) == false)
		{
			Q.push(pixel{R.x-1,R.y});
			explored(R.x-1,R.y) = true;

		}
		if (R.y+1 < I.height() && I(R.x,R.y+1) == original_color && explored(R.x,R.y+1) == false)
		{
			Q.push(pixel{R.x,R.y+1});
			explored(R.x,R.y+1) = true;
		}
		if (R.y > 0 && I(R.x,R.y-1) == original_color && explored(R.x,R.y-1) == false)
		{
			Q.push(pixel{R.x,R.y-1});
			explored(R.x,R.y-1) = true;
		}
...
int main()
{
	...
	for (size_t m = 0; m <= max_size; ++m)
	{
		for (size_t n = 0; n <= max_size; ++n)
		{
			...
			std::cout << "passed random tests for images of size " << m << "×" << n << std::endl;
		}
	}
